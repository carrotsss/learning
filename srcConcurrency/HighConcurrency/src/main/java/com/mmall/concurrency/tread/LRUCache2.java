...
/**
 * 数据时直接利用HashMap来存放的。
 * 内部使用一个双向链表来存放数据，所以有一个头结点header，以及尾节点tailer
 * 每次写入头结点，删除尾节点时都是依赖于header，tailer
 * 使用数据移动到链表头时，第一步是需要在双向链表中找到该节点。这里就体现出链表问题了。查找效率低，最差需要o（n）之后依赖于当前节点进行移动。
 * 在写入头结点时有判断链表大小等于2时需要删除初始化的头尾节点。这是因为初始化时生成了两个双向节点么没有数据只是为了形成一个数据结构。当真实数据进来时候需要删除以方便后续的操作。
 * 以上所有操作都是线程不安全的，需要使用者自行控制。
 *
 * @since JDK 1.8
 */
