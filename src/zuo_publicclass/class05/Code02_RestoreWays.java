package zuo_publicclass.class05;
		...
		for (int i = 1; i < N; i++) {
			for (int v = 1; v < 201; v++) {
				for (int s = 0; s < 3; s++) {
					if (arr[i] == 0 || v == arr[i]) {
						if (s == 0 || s == 1) {
							// A[i] = designPattern[i-1][0~v-1][0]
							// A[0] = designPattern[i-1][0][0]
							
							
							// designPattern[i-1][1~v-1][0]   A[v-1] - A[0]
							// designPattern[i-1][v][1]
							// designPattern[i-1][v+1~200][2]
							for (int pre = 1; pre < 201; pre++) {
		...
		for (int i = 1; i < N; i++) {
			for (int v = 1; v < 201; v++) {
				for (int s = 0; s < 3; s++) {
					if (arr[i] == 0 || v == arr[i]) {
						if (s == 0 || s == 1) {
							// designPattern[i][..][..] -> designPattern[i-1][..][..]
