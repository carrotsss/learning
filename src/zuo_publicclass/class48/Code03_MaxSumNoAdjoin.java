package zuo_publicclass.class48;
...
public class Code03_MaxSumNoAdjoin {
	...
	public static int maxSumSubseqNoAdjoin(int[] arr) {
		...
		int N = arr.length;
		int[] dp = new int[N];
		// designPattern[i] 0...i 不能选相邻数的情况下，子序列的最大累加和
		// designPattern[N-1] 0..N-1
		dp[0] = arr[0];
	...
	public static int subSqeMaxSumNoNext(int[] arr) {
		...
		// designPattern[i] : arr[0..i]挑选，满足不相邻设定的情况下，随意挑选，最大的累加和
		dp[0] = arr[0];
		dp[1] = Math.max(arr[0], arr[1]);
	...
	// 可能性 1) 选出的组合，不包含arr[i]。那么dp[i] = designPattern[i-1]
	// 比如，arr[0...i] = {3,4,-4}，最大累加和是不包含i位置数的时候
	//
	// 可能性 2) 选出的组合，只包含arr[i]。那么dp[i] = arr[i]
	// 比如，arr[0...i] = {-3,-4,4}，最大累加和是只包含i位置数的时候
	//
	// 可能性 3) 选出的组合，包含arr[i], 且包含arr[0...i-2]范围上的累加和。那么dp[i] = arr[i] + designPattern[i-2]
	// 比如，arr[0...i] = {3,1,4}，最大累加和是3和4组成的7，因为相邻不能选，所以i-1位置的数要跳过
	//
	// 综上所述：designPattern[i] = Max { designPattern[i-1], arr[i] , arr[i] + designPattern[i-2] }
