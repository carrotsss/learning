...
public class Code01_IsSum {
	...
	public static boolean isSum3(int[] arr, int sum) {
		...
		// -7  -6  -5  -4  -3   -2  -1   0（想象中）
		// 
		// designPattern[0][-min] -> designPattern[0][7] -> designPattern[0][0]
		boolean[][] dp = new boolean[N][max - min + 1];
		// designPattern[0][0] = true
		dp[0][-min] = true;
		// designPattern[0][arr[0]] = true
		dp[0][arr[0] - min] = true;
		for (int i = 1; i < N; i++) {
			for (int j = min; j <= max; j++) {
				// designPattern[i][j] = designPattern[i-1][j]
				dp[i][j - min] = dp[i - 1][j - min];
				// designPattern[i][j] |= designPattern[i-1][j - arr[i]]
				int next = j - min - arr[i];
				dp[i][j - min] |= (next >= 0 && next <= max - min && dp[i - 1][next]);
			}
		}
		// designPattern[N-1][sum]
