...
public class Code07_TargetSum {
	...
	// 该方案中取了正的集合为P = {1，3，5}
	// 该方案中取了负的集合为N = {2，4}
	// 所以任何一种方案，都一定有 sum(P746_MinCostClimbingStairs) - sum(N) = target
	// 现在我们来处理一下这个等式，把左右两边都加上sum(P746_MinCostClimbingStairs) + sum(N)，那么就会变成如下：
	// sum(P746_MinCostClimbingStairs) - sum(N) + sum(P746_MinCostClimbingStairs) + sum(N) = target + sum(P746_MinCostClimbingStairs) + sum(N)
	// 2 * sum(P746_MinCostClimbingStairs) = target + 数组所有数的累加和
	// sum(P746_MinCostClimbingStairs) = (target + 数组所有数的累加和) / 2
	...
	public static int subset1(int[] nums, int s) {
		int n = nums.length;
		// designPattern[i][j] : nums前缀长度为i的所有子集，有多少累加和是j？
	...
	// 因为如果 i - n < 0，designPattern[i]怎么更新？和上一步的dp[i]一样！所以不用更新
	// 如果 i - n >= 0，designPattern[i]怎么更新？上一步的dp[i] + 上一步dp[i - n]的值，这才需要更新
